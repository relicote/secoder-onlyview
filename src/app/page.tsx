...
import { Facebook, Instagram, Twitter, Youtube, Mail, Phone, MoveRight, Bell, ShieldCheck, HelpCircle } from 'lucide-react';
  ...
  <header className="fixed top-0 left-0 right-0 z-50">
    <div className="container mx-auto flex h-[91px] w-full max-w-[1920] items-center justify-around gap-2 sm:px-6 lg:px-8">
      ...
      <div className="flex items-center justify-end gap-4">
        ...
        <Link href="#" aria-label="Twitter"><Twitter className="h-5 w-5 text-foreground/80 hover:text-primary transition-colors" /></Link>
      </div>
    </div>
  </header>
);
    ...
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
        <div className="flex flex-col gap-4">
          <Link href="/" className="text-2xl font-black tracking-wider text-primary font-headline">SECODER</Link>
          <p className="text-sm text-muted-foreground">Soluções de cibersegurança para um mundo digital mais seguro.</p>
        ...
        <div className="flex flex-col gap-4">
          ...
          <Link href="#tools" className="text-sm text-muted-foreground hover:text-primary transition-colors">Nossas Ferramentas</Link>
          <Link href="#specialties" className="text-sm text-muted-foreground hover:text-primary transition-colors">Nossas Especialidades</Link>
          <Link href="#faq" className="text-sm text-muted-foreground hover:text-primary transition-colors">Perguntas Frequentes</Link>
        </div>

        <div className="col-span-1 lg:col-span-2 flex flex-col gap-4">
          <h3 className="font-semibold text-foreground">Assine nossa Newsletter</h3>
          <p className="text-sm text-muted-foreground">Receba as últimas notícias e insights sobre cibersegurança.</p>
          <form className="flex gap-2">
            <Input type="email" placeholder="Seu melhor e-mail" className="bg-background" />
            <Button type="submit" className="whitespace-nowrap">Enviar</Button>
          </form>
        </div>
      </div>

      <div className="mt-12 pt-8 border-t border-border/50 flex flex-col sm:flex-row justify-between items-center gap-4">
        <p className="text-sm text-muted-foreground">&copy; {new Date().getFullYear()} Secoder. Todos os direitos reservados.</p>
  ...
  const tools = [
    {
      ...
      description: 'Nossa plataforma monitora continuamente seus sistemas para detectar e alertar sobre ameaças em tempo real, 24/7.',
      imageId: 'tool-1',
    },
    {
      title: 'Scanner de Vulnerabilidades',
      description: 'Identifique e corrija falhas de segurança em suas aplicações e infraestrutura antes que sejam exploradas por invasores.',
      imageId: 'tool-2',
    },
    {
      title: 'Simulador de Phishing',
      description: 'Treine sua equipe para reconhecer e reagir a ataques de phishing com simulações realistas e campanhas educativas.',
      imageId: 'tool-3',
    },
    {
      title: 'Base de Conhecimento',
      description: 'Acesse um portal completo com as melhores práticas, guias de segurança e insights para fortalecer suas defesas.',
      imageId: 'tool-4',
    },
  ];
  ...
  const faqItems = [
    {
      ...
      answer: "Um Pentest é um ataque cibernético simulado e autorizado em um sistema de computador, realizado para avaliar a segurança do sistema. O teste identifica pontos fracos (vulnerabilidades), bem como pontos fortes, permitindo que uma avaliação de segurança completa seja concluída."
    },
    {
      question: "Como funciona a simulação de phishing da Secoder?",
      answer: "Nossa plataforma cria e envia e-mails de phishing realistas e inofensivos para seus funcionários. Analisamos as taxas de cliques e relatamos os resultados, fornecendo treinamento direcionado para ajudar a educar sua equipe e reduzir os riscos de ataques de engenharia social."
    },
    {
      question: "O monitoramento de ameaças é realmente em tempo real?",
      answer: "Sim. Nossas ferramentas de monitoramento analisam o tráfego de rede e os logs do sistema continuamente, 24/7. Usamos algoritmos avançados e IA para detectar atividades suspeitas e anomalias assim que ocorrem, permitindo uma resposta imediata a possíveis ameaças."
    },
    {
      question: "Quais são os principais riscos de segurança que as empresas enfrentam hoje?",
      answer: "Atualmente, os principais riscos incluem ataques de ransomware, phishing e engenharia social, vulnerabilidades em softwares desatualizados, e ameaças internas. A Secoder oferece soluções abrangentes para mitigar todos esses riscos e proteger seus ativos digitais."
    }
  ];
    ...
    elimine riscos
        ...
        <section id="specialties" className="py-20 bg-background">
            <div className="container mx-auto px-4 sm:px-6 lg:px-8 text-center flex flex-col items-center">
                ...
                <p className="mt-4 max-w-2xl mx-auto text-muted-foreground">Nossa equipe é certificada pelas mais renomadas instituições de segurança do mundo.</p>
        ...
        <section id="tools" className="py-20 bg-gray-900/50">
          <div className="container mx-auto px-4 sm:px-6 lg:px-8">
            <h2 className="text-3xl md:text-4xl font-bold text-center text-foreground font-headline">Nossas Ferramentas</h2>
            <div className="mt-12 grid gap-8 sm:grid-cols-2 lg:grid-cols-2">
              {tools.map((tool, index) => {
                const image = PlaceHolderImages.find(img => img.id === tool.imageId);
                return (
                  <Card key={index} className="bg-card border-border/50 overflow-hidden transform hover:-translate-y-2 transition-transform duration-300 flex flex-col">
                    <CardHeader>
                      <CardTitle className="text-primary text-xl font-bold">{tool.title}</CardTitle>
                    </CardHeader>
        ...
        <section className="py-20 bg-background">
            <div className="container mx-auto px-4 sm:px-6 lg:px-8 flex flex-col items-center gap-8">
                <h2 className="text-3xl md:text-4xl font-bold max-w-3xl mx-auto text-foreground font-headline text-center">
                    Garanta que seu time saiba se prevenir de ataques de engenharia social.
                </h2>
                    ...
                    <Card className="bg-card border-border/50 p-8 rounded-2xl">
                        <div className="grid md:grid-cols-3 gap-8 items-center">
                            <div className="flex items-center gap-6 md:col-span-2">
                                ...
                                <div>
                                    <Badge variant="default" className="mb-2 bg-primary/10 text-primary border border-primary/20">
                                        <Bell className="mr-1 h-3 w-3" />
                                        Online
                                    </Badge>
                                    <h3 className="text-2xl font-bold font-headline text-foreground">Teste de phishing</h3>
                                </div>
                            </div>
                            <p className="text-muted-foreground text-sm md:col-span-3">
                                Teste avançado de phishing para identificar todas as vulnerabilidades no projeto e mitigar qualquer risco associado. O objetivo é detectar e eliminar quaisquer falhas humanas que possam ser exploradas por agentes maliciosos. Vamos garantir que todas as medidas de segurança sejam implementadas de forma eficaz para proteger seu sistema e dados contra ataques cibernéticos.
                            </p>
                        </div>
                    </Card>
                    <div className="mt-4 flex justify-center">
                         <Button size="lg" className="group rounded-md bg-primary hover:bg-primary/90 text-primary-foreground text-lg font-bold px-8 py-6 transition-all duration-300 hover:scale-105 shadow-lg shadow-primary/20 w-full">
        ...
        <section id="faq" className="py-20 bg-gray-900/50">
          <div className="container mx-auto px-4 sm:px-6 lg:px-8">
            <h2 className="text-3xl md:text-4xl font-bold text-center text-foreground font-headline">Perguntas Frequentes</h2>
            <Accordion type="single" collapsible className="w-full max-w-4xl mx-auto mt-12 grid md:grid-cols-2 gap-x-8 gap-y-4">
              {faqItems.map((item, index) => (
                <AccordionItem key={index} value={`item-${index}`} className="bg-card border-border/50 rounded-lg px-4">
                  ...
                  <AccordionContent className="text-muted-foreground pl-16 pr-4">{item.answer}</AccordionContent>
                </AccordionItem>
              ))}
            </Accordion>
          </div>
        ...
        <section className="py-20 bg-background">
          <div className="container mx-auto px-4 sm:px-6 lg:px-8 text-center">
            <h2 className="text-3xl md:text-4xl font-bold text-foreground font-headline">Pronto para fortalecer sua segurança?</h2>
            <p className="mt-4 max-w-2xl mx-auto text-muted-foreground">Nossa equipe de especialistas está pronta para ajudar você a proteger seu negócio.</p>
